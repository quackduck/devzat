...
func pluginsCMD(_ string, u *User) {
	...
	u.room.broadcast(NewNoSenderMessage("Plugin commands  \n" + autogenerated))
}
...
func lsTokensCMD(_ string, u *User) {
	...
	if len(Tokens) == 0 {
		u.room.broadcast(NewDevbotMessage("No tokens found."))
		return
	}
	msg := "Tokens:  \n"
	...
	u.writeln(NewDevbotMessage(msg))
}
...
func revokeTokenCMD(rest string, u *User) {
	...
	if len(rest) == 0 {
		u.room.broadcast(NewDevbotMessage("Please provide a sha256 hash of a token to revoke."))
		return
	}
	for token := range Tokens {
		if shasum(token) == rest {
			delete(Tokens, token)
			saveTokens()
			u.room.broadcast(NewDevbotMessage("Token revoked!"))
			return
		}
	}
	u.room.broadcast(NewDevbotMessage("Token not found."))
}
...
func grantTokenCMD(rest string, u *User) {
	...
	if err != nil {
		u.room.broadcast(NewDevbotMessage("Error generating token: " + err.Error()))
		Log.Println(err)
		return
	}
	...
	if len(split) > 0 && len(split[0]) > 0 && split[0][0] == '@' {
		...
		if ok {
			toUser.writeln(NewDevbotMessage("You have been granted a token: " + token))
		} else {
	...
	Tokens[token] = rest
	u.writeln(NewDevbotMessage("Granted token: " + token))
	saveTokens()
}
