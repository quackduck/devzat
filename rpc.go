...
func (s *pluginServer) SendMessage(ctx context.Context, msg *pb.Message) (*pb.MessageRes, error) {
	...
	if msg.GetEphemeralTo() != "" {
		...
		u.writeln(NewMessage(msg.GetFrom()+" -> ", msg.Msg, PrivateMessageReceive))
	} else {
		r := Rooms[msg.Room]
		if r == nil {
			return nil, status.Error(codes.InvalidArgument, "Room does not exist")
		}
		r.broadcast(NewMessage(msg.GetFrom(), msg.Msg, FromPluginMessage))
	}
...
func pluginsCMD(_ string, u *User) {
	...
	u.room.broadcast(NewMessage("", "Plugin commands  \n"+autogenerated, NoSenderMessage))
}
...
func lsTokensCMD(_ string, u *User) {
	...
	if len(Tokens) == 0 {
		u.room.broadcast(NewMessage(Devbot, "No tokens found."))
		return
	}
	msg := "Tokens:  \n"
	...
	u.writeln(NewMessage(Devbot, msg))
}
...
func revokeTokenCMD(rest string, u *User) {
	...
	if len(rest) == 0 {
		u.room.broadcast(NewMessage(Devbot, "Please provide a sha256 hash of a token to revoke."))
		return
	}
	for token := range Tokens {
		if shasum(token) == rest {
			delete(Tokens, token)
			saveTokens()
			u.room.broadcast(NewMessage(Devbot, "Token revoked!"))
			return
		}
	}
	u.room.broadcast(NewMessage(Devbot, "Token not found."))
}
...
func grantTokenCMD(rest string, u *User) {
	...
	if err != nil {
		u.room.broadcast(NewMessage(Devbot, "Error generating token: "+err.Error()))
		Log.Println(err)
		return
	}
	...
	if len(split) > 0 && len(split[0]) > 0 && split[0][0] == '@' {
		...
		if ok {
			toUser.writeln(NewMessage(Devbot, "You have been granted a token: "+token))
		} else {
	...
	Tokens[token] = rest
	u.writeln(NewMessage(Devbot, "Granted token: "+token))
	saveTokens()
}
