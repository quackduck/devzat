...
func dmCMD(rest string, u *User) {
	...
	if len(restSplit) < 2 {
		u.writeln(NewMessage(Devbot, "You gotta have a message, mate"))
		return
	}
	peer, ok := findUserByName(u.room, restSplit[0])
	if !ok {
		u.writeln(NewMessage(Devbot, "No such person lol, who you wanna dm? (you might be in the wrong room)"))
		return
	}
	msg := strings.TrimSpace(strings.TrimPrefix(rest, restSplit[0]))
	u.writeln(NewMessage(peer.Name+" <- ", msg, PrivateMessageSend))
	if u == peer {
	...
	peer.writeln(NewMessage(u.Name+" -> ", msg, PrivateMessageReceive))
}
...
func hangCMD(rest string, u *User) {
	if len([]rune(rest)) > 1 {
		if !u.isBridge {
			...
			u.writeln(NewMessage(Devbot, "(that word won't show dw)"))
		}
		hangGame = &hangman{rest, 15, " "} // default value of guesses so empty space is given away
		u.room.broadcast(NewMessage(Devbot, u.Name+" has started a new game of Hangman! Guess letters with hang <letter>"))
		u.room.broadcast(NewMessage(Devbot, "```\n"+hangPrint(hangGame)+"\nTries: "+strconv.Itoa(hangGame.triesLeft)+"\n```"))
		return
	}
	if !u.isBridge {
	...
	if strings.Trim(hangGame.word, hangGame.guesses) == "" {
		u.room.broadcast(NewMessage(Devbot, "The game has ended. Start a new game with hang <word>"))
		return
	}
	if len(rest) == 0 {
		u.room.broadcast(NewMessage(Devbot, "Start a new game with hang <word> or guess with hang <letter>"))
		return
	}
	if hangGame.triesLeft == 0 {
		u.room.broadcast(NewMessage(Devbot, "No more tries! The word was "+hangGame.word))
		return
	}
	if strings.Contains(hangGame.guesses, rest) {
		u.room.broadcast(NewMessage(Devbot, "You already guessed "+rest))
		return
	}
	...
	u.room.broadcast(NewMessage(Devbot, "```\n"+display+"\nTries: "+strconv.Itoa(hangGame.triesLeft)+"\n```"))
	if strings.Trim(hangGame.word, hangGame.guesses) == "" {
		u.room.broadcast(NewMessage(Devbot, "You got it! The word was "+hangGame.word))
...
func usersCMD(_ string, u *User) {
	u.room.broadcast(NewMessage("", printUsersInRoom(u.room), NoSenderMessage))
}

func dmRoomCMD(line string, u *User) {
	u.writeln(NewMessage(u.messaging.Name+" <- ", line, PrivateMessageSend))
	if u == u.messaging {
		devbotRespond(u.room, []string{"You must be really lonely, DMing yourself.",
			"Don't worry, I won't judge :wink:",
			"srsly?",
	...
	u.messaging.writeln(NewMessage(u.Name+" -> ", line, PrivateMessageReceive))
}
...
func devmonkCMD(_ string, u *User) {
	...
	u.writeln(NewMessage(Devbot, "Okay type this text: \n\n> "+text))
	u.term.SetPrompt("> ")
	defer u.formatPrompt()
	start := time.Now()
	line, err := u.term.ReadLine()
	if err == term.ErrPasteIndicator { // TODO: doesn't work for some reason?
		u.room.broadcast(NewMessage(Devbot, "SMH did you know that "+u.Name+" tried to cheat in a typing game?"))
		return
	}
	...
	u.room.broadcast(NewMessage(Devbot, "Okay "+u.Name+", you typed that in "+dur.Truncate(time.Second/10).String()+" so your speed is "+
		strconv.FormatFloat(
...
func ticCMD(rest string, u *User) {
	if rest == "" {
		u.room.broadcast(NewMessage(Devbot, "Starting a new game of Tic Tac Toe! The first player is always X."))
		u.room.broadcast(NewMessage(Devbot, "Play using tic <cell num>"))
	...
	if err != nil {
		u.room.broadcast(NewMessage(Devbot, "Make sure you're using a number lol"))
		return
	}
	if m < 1 || m > 9 {
		u.room.broadcast(NewMessage(Devbot, "Moves are numbers between 1 and 9!"))
		return
	}
	err = tttGame.Apply(tictactoe.Move(m-1), currentPlayer)
	if err != nil {
		u.room.broadcast(NewMessage(Devbot, err.Error()))
		return
	}
	...
	if !(tttGame.Condition() == tictactoe.NotEnd) {
		u.room.broadcast(NewMessage(Devbot, tttGame.Condition().String()))
		currentPlayer = tictactoe.X
		tttGame = new(tictactoe.Board)
	}
}
...
func bellCMD(rest string, u *User) {
	...
	case "off":
		...
		u.room.broadcast(NewMessage("", "bell off (never)", NoSenderMessage))
	case "on":
		u.Bell = true
		u.PingEverytime = false
		u.room.broadcast(NewMessage("", "bell on (pings)", NoSenderMessage))
	case "all":
		u.Bell = true
		u.PingEverytime = true
		u.room.broadcast(NewMessage("", "bell all (every message)", NoSenderMessage))
	case "", "status":
		if u.PingEverytime {
			u.room.broadcast(NewMessage("", "bell all (every message)", NoSenderMessage))
		} else if u.Bell {
	...
	default:
		u.room.broadcast(NewMessage(Devbot, "your options are off, on and all"))
	}
}
...
func cdCMD(rest string, u *User) {
	...
	if u.messaging != nil {
		u.messaging = nil
		u.writeln(NewMessage(Devbot, "Left private chat"))
		if rest == "" || rest == ".." {
			return
		}
	}
	...
	if strings.HasPrefix(rest, "#") {
		...
		if len(rest) > MaxRoomNameLen {
			...
			u.room.broadcast(NewMessage(Devbot, "Room name lengths are limited, so I'm shortening it to "+rest+"."))
		}
	...
	if rest == "" {
		...
		u.room.broadcast(NewMessage("", "Rooms and users  \n"+strings.TrimSpace(roomsInfo), NoSenderMessage))
		return
	}
	name := strings.Fields(rest)[0]
	if len(name) == 0 {
		u.writeln(NewMessage(Devbot, "You think people have empty names?"))
		return
	}
	peer, ok := findUserByName(u.room, name)
	if !ok {
		u.writeln(NewMessage(Devbot, "No such person lol, who do you want to dm? (you might be in the wrong room)"))
		return
	}
	u.messaging = peer
	u.writeln(NewMessage(Devbot, "Now in DMs with "+peer.Name+". To leave use cd .."))
}
...
func tzCMD(tzArg string, u *User) {
	...
	if tzArg == "" {
		...
		u.room.broadcast(NewMessage(Devbot, "Enabled relative times!"))
		return
	}
	...
	if err != nil {
		u.room.broadcast(NewMessage(Devbot, "Weird timezone you have there, use the format Continent/City, the usual US timezones (PST, PDT, EST, EDT...) or check nodatime.org/TimeZones!"))
		return
	}
	u.FormatTime24 = len(tzArgList) == 2 && tzArgList[1] == "24h"
	u.room.broadcast(NewMessage(Devbot, "Changed your timezone!"))
}

func bioCMD(line string, u *User) {
	if line == "" {
		u.writeln(NewMessage(Devbot, "Your current bio is:  \n> "+u.Bio))
		u.term.SetPrompt("> ")
		defer u.formatPrompt()
		for {
			...
			if input != "" {
				if len(input) > MaxBioLen {
					u.writeln(NewMessage(Devbot, "Your bio is too long. It shouldn't be more than "+strconv.Itoa(MaxBioLen)+" characters."))
				}
				u.Bio = input
	...
	if !ok {
		u.room.broadcast(NewMessage(Devbot, "Who???"))
		return
	}
	u.room.broadcast(NewMessage(Devbot, target.Bio, NoSenderMessage))
}

func idCMD(line string, u *User) {
	victim, ok := findUserByName(u.room, line)
	if !ok {
		u.room.broadcast(NewMessage(Devbot, "User not found", NoSenderMessage))
		return
	}
	u.room.broadcast(NewMessage(Devbot, victim.id, NoSenderMessage))
}
...
func promptCMD(line string, u *User) {
	...
	u.formatPrompt()
	if line == "" {
		u.writeln(NewMessage(Devbot, "(Your prompt is now empty. Did you mean to get more info about your prompt? Run `man prompt` for more info)"))
	}
}
...
func listBansCMD(_ string, u *User) {
	...
	u.room.broadcast(NewMessage(Devbot, msg))
}

func unbanCMD(toUnban string, u *User) {
	if !auth(u) {
		u.room.broadcast(NewMessage(Devbot, "Not authorized"))
		return
	}

	if unbanIDorIP(toUnban) {
		u.room.broadcast(NewMessage(Devbot, "Unbanned person: "+toUnban))
		saveBans()
	} else {
		u.room.broadcast(NewMessage(Devbot, "I couldn't find that person"))
	}
}
...
func banCMD(line string, u *User) {
	...
	if len(split) == 0 {
		u.room.broadcast(NewMessage(Devbot, "Which user do you want to ban?"))
		return
	}
	var victim *User
	var ok bool
	...
	if split[0] == "devbot" {
		u.room.broadcast(NewMessage(Devbot, "Do you really think you can ban me, puny human?"))
...
func kickCMD(line string, u *User) {
	...
	if !ok {
		if line == "devbot" {
			u.room.broadcast(NewMessage(Devbot, "You will pay for this"))
...
func colorCMD(rest string, u *User) {
	if rest == "which" {
		u.room.broadcast(NewMessage(Devbot, u.Color+" "+u.ColorBG))
...
func helpCMD(_ string, u *User) {
	u.room.broadcast(NewMessage("", `Welcome to Devzat! Devzat is chat over SSH: github.com/quackduck/devzat  
...
Thanks to Caleb Denio for lending his server!`, NoSenderMessage))
}

func catCMD(line string, u *User) {
	if line == "" {
		u.room.broadcast(NewMessage(Devbot, "usage: cat [-benstuv] [file ...]", NoSenderMessage))
	} else if line == "README.md" {
		helpCMD(line, u)
	} else {
		u.room.broadcast(NewMessage(Devbot, "cat: "+line+": Permission denied", NoSenderMessage))
	}
}

func rmCMD(line string, u *User) {
	if line == "" {
		u.room.broadcast(NewMessage("", `usage: rm [-f | -i] [-dPRrvW] file ...
unlink file`, NoSenderMessage))
	} else {
		u.room.broadcast(NewMessage(Devbot, "rm: "+line+": Permission denied, sucker", NoSenderMessage))
	}
}

func exampleCodeCMD(line string, u *User) {
	if line == "big" {
		u.room.broadcast(NewMessage(Devbot, "```go\npackage main\n\nimport \"fmt\"\n\nfunc sum(nums ...int) {\n    fmt.Print(nums, \" \")\n    total := 0\n    for _, num := range nums {\n        total += num\n    }\n    fmt.Println(total)\n}\n\nfunc main() {\n\n    sum(1, 2)\n    sum(1, 2, 3)\n\n    nums := []int{1, 2, 3, 4}\n    sum(nums...)\n}\n```"))
		return
	}
	u.room.broadcast(NewMessage(Devbot, "\n```go\npackage main\nimport \"fmt\"\nfunc main() {\n   fmt.Println(\"Example!\")\n}\n```"))
}
...
func themeCMD(line string, u *User) {
	...
	u.room.broadcast(NewMessage(Devbot, "Themes do not currently work because Devzat is switching to using glamour for rendering."))
	if line == "list" {
		u.room.broadcast(NewMessage(Devbot, "Available themes: "+strings.Join(chromastyles.Names(), ", ")))
		return
	}
	for _, name := range chromastyles.Names() {
		if name == line {
			//markdown.CurrentTheme = chromastyles.Get(name)
			u.room.broadcast(NewMessage(Devbot, "Theme set to "+name))
			return
		}
	}
	u.room.broadcast(NewMessage(Devbot, "What theme is that? Use theme list to see what's available."))
}

func asciiArtCMD(_ string, u *User) {
	u.room.broadcast(NewMessage(Devbot, Art, NoSenderMessage))
}

func pwdCMD(_ string, u *User) {
	if u.messaging != nil {
		u.writeln(NewMessage("", u.messaging.Name, NoSenderMessage))
	} else {
		u.room.broadcast(NewMessage(Devbot, u.room.name, NoSenderMessage))
	}
}
...
func pronounsCMD(line string, u *User) {
	...
	if line == "" {
		u.room.broadcast(NewMessage(Devbot, "Set pronouns by providing em or query a user's pronouns!"))
		return
	}
	...
	if len(args) == 1 && strings.HasPrefix(args[0], "@") {
		...
		if !ok {
			u.room.broadcast(NewMessage(Devbot, "Who's that?"))
			return
		}
		u.room.broadcast(NewMessage(Devbot, victim.Name+"'s pronouns are "+victim.displayPronouns()))
		return
	}

	u.Pronouns = strings.Fields(strings.ReplaceAll(strings.ToLower(line), "\n", ""))
	//u.changeColor(u.Color) // refresh pronouns
	u.room.broadcast(NewMessage(Devbot, u.Name+" now goes by "+u.displayPronouns()))
}

func emojisCMD(_ string, u *User) {
	u.room.broadcast(NewMessage(Devbot, `See the complete list at https://github.com/ikatyang/emoji-cheat-sheet/  
...
func commandsRestCMD(_ string, u *User) {
	u.room.broadcast(NewMessage(Devbot, "The rest  \n"+autogenCommands(RestCMDs), NoSenderMessage))
}

func manCMD(rest string, u *User) {
	if rest == "" {
		u.room.broadcast(NewMessage(Devbot, "What command do you want help with?"))
		return
	}

	if rest == "prompt" {
		u.room.broadcast(NewMessage(Devbot, `prompt <prompt> sets your prompt
...
The default prompt is "\u:\S".`))
	...
	if cmd, ok := getCMD(rest); ok {
		u.room.broadcast(NewMessage(Devbot, "Usage: "+cmd.name+" "+cmd.argsInfo+"  \n"+cmd.info))
		return
	}
	// Plugin commands
	if c, ok := PluginCMDs[rest]; ok {
		u.room.broadcast(NewMessage(Devbot, "Usage: "+rest+" "+c.argsInfo+"  \n"+c.info))
		return
	}

	u.room.broadcast(NewMessage(Devbot, "This system has been minimized by removing packages and content that are not required on a system that users do not log into.\n\nTo restore this content, including manpages, you can run the 'unminimize' command. You will still need to ensure the 'man-db' package is installed.", NoSenderMessage))
}
...
func lsCMD(rest string, u *User) {
	if len(rest) > 0 && rest[0] == '#' {
		if r, ok := Rooms[rest]; ok {
			...
			u.room.broadcast(NewMessage(Devbot, usersList, NoSenderMessage))
			return
		}
	}
	if rest == "-i" { // show ids
		...
		u.room.broadcast(NewMessage(Devbot, s, NoSenderMessage))
		return
	}
	if rest != "" {
		u.room.broadcast(NewMessage(Devbot, "ls: "+rest+" Permission denied", NoSenderMessage))
		return
	}
	roomList := ""
	for _, r := range Rooms {
	...
	u.room.broadcast(NewMessage(Devbot, "README.md "+usersList+roomList, NoSenderMessage))
}

func commandsCMD(_ string, u *User) {
	u.room.broadcast(NewMessage(Devbot, "Commands  \n"+autogenCommands(MainCMDs), NoSenderMessage))
}

func unameCMD(rest string, u *User) {
	if unameCommit == "" || unameTime == "" {
		u.room.broadcast(NewMessage(Devbot, "No uname output available. Build Devzat with `"+color.HiYellowString(`go build -ldflags "-X 'main.unameCommit=$(git rev-parse HEAD)' -X 'main.unameTime=$(date)'"`)+"` to enable.", NoSenderMessage))
		return
	}
	u.room.broadcast(NewMessage(Devbot, "Devzat ("+unameCommit+") "+unameTime, NoSenderMessage))
}

func uptimeCMD(rest string, u *User) {
	uptime := time.Since(StartupTime)
	u.room.broadcast(NewMessage(Devbot, fmt.Sprintf("up %v days, %02d:%02d:%02d", int(uptime.Hours()/24), int(math.Mod(uptime.Hours(), 24)), int(math.Mod(uptime.Minutes(), 60)), int(math.Mod(uptime.Seconds(), 60))), NoSenderMessage))
}

func neofetchCMD(_ string, u *User) {
	content, err := os.ReadFile(Config.DataDir + "/neofetch.txt")
	if err != nil {
		u.room.broadcast(NewMessage(Devbot, "Error reading "+Config.DataDir+"/neofetch.txt: "+err.Error(), NoSenderMessage))
		return
	}
	...
	u.room.broadcast(NewMessage(Devbot, result, NoSenderMessage))
}
...
func rmdirCMD(rest string, u *User) {
	if rest == "#main" {
		u.room.broadcast(NewMessage(Devbot, "rmdir: failed to remove '"+rest+"': Operation not permitted", NoSenderMessage))
	} else if room, ok := Rooms[rest]; ok {
		if len(room.users) == 0 {
			delete(Rooms, rest)
			u.room.broadcast(NewMessage(Devbot, "rmdir: removing directory, '"+rest+"'", NoSenderMessage))
		} else {
			u.room.broadcast(NewMessage(Devbot, "rmdir: failed to remove '"+rest+"': Room not empty", NoSenderMessage))
		}
	} else {
		u.room.broadcast(NewMessage(Devbot, "rmdir: failed to remove '"+rest+"': No such room", NoSenderMessage))
	}
}
...
func logsCMD(rest string, u *User) {
	...
	for _, line := range GetAdminLog(lineCount) {
		u.writeln(NewMessage(Devbot, line))
	}
